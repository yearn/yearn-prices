...
The fallback shares the request-path DeFiLlama client with `batchHistorical`: each upstream call is capped at 2.5 seconds and DeFiLlama `429` responses are not retried, so a rate-limited lookup fails on the first attempt instead of backing off. Offline jobs keep the retrying client.
