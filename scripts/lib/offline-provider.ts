...
export function offlineProvider(rps: number) {
  ...
  return new BatchedHistoricalClient(
    new DefiLlamaClient(
      new Limiter(rps, 1000),
      (_attempt, delay, _url, status) => {
        if (status === 429) cooldown = Math.max(cooldown, Date.now() + Math.max(delay, 60000))
      },
      { timeoutMs: 10000, honorRetryAfter: true, retryRateLimits: true, retryAfterCapMs: 43200000 }
    )
  )
}
