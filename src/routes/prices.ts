...
export async function handleHistorical(
  ...
  const record = await getExactHistoricalPrice(
    pool,
...
export async function handleSpot(
  ...
  const requests = parseSpotCoins(
    new URL(request.url).searchParams.get("coins"),
  );
  ...
  for (const requestRange of requests) {
    for (const timestamp of normalizedDaysInRange(
      ...
      expectedTimestamps.add(
        `${requestRange.chain}:${requestRange.token}:${timestamp}`,
      );
    }
  }
...
function toExactKey(
  ...
  const timestamps = normalizedDaysInRange(
