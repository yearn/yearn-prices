...
const ALIASES: readonly DefiLlamaCoinGeckoAlias[] = [
  ...
  fantomProxy({
    ...
    rationale: 'Abracadabra documented Fantom MIM as issuer-backed MIM bridged by Anyswap; no asset-specific July 6 reserve outflow was established.',
    references: [
...
const ALIAS_BY_TOKEN = new Map(
  ALIASES.map(alias => [aliasKey(alias.chain, alias.token), alias] as const),
)

export function getDefiLlamaCoinGeckoAlias(
  chain: string,
  token: string,
): DefiLlamaCoinGeckoAlias | null {
