...
export function yearnShareAdapter(options: OnchainAdapterOptions): PlannedPriceAdapter {
  return plannedAdapter('yearn-share-rate', async (target) => {
    ...
    return {
      dependencies: [
        { target: childTarget(target, underlying, state.numericBlockNumber), label: 'Yearn share underlying' }
      ],
      metadata: conversion,
      evaluate([input]) {
        return {
