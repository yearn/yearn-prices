 ...
 */
export function nativeShareAdapter(options: OnchainAdapterOptions): PlannedPriceAdapter {
  return plannedAdapter('native-share-convert-to-assets', async (target) => {
    ...
    return {
      dependencies: [
        {
          target: childTarget(target, nativeAsset, state.numericBlockNumber),
          label: 'native share underlying'
        }
      ],
      metadata: conversion,
      evaluate([input]) {
        return {
          priceUsd: calculateWrapperPrice(convertedAssetsRaw, 18, oneShareRaw, shareDecimals, input.priceUsd),
