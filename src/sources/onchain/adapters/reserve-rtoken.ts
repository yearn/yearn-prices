 ...
 */
export function reserveRTokenAdapter(options: OnchainAdapterOptions): PlannedPriceAdapter {
  return plannedAdapter('reserve-rtoken-redemption', async (target) => {
    ...
    return {
      dependencies: constituents
        ...
        .map((address) => ({
          ...
          label: 'Reserve RToken redemption constituent'
        })),
      metadata,
      evaluate(inputs) {
        return {
