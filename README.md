...
Pushing to `main` runs `.github/workflows/deploy.yml`: a `prepare` job loads `DATABASE_URL` and RPC URLs from Doppler via OIDC (`DOPPLER_PREPARE_IDENTITY_ID` repo var), runs migrations and warms the price cache; then the reusable `yearn/yearn-gha` Cloudflare deploy workflow fetches the Cloudflare token from Doppler via OIDC (`DOPPLER_PRODUCTION_IDENTITY_ID` repo var) and runs `wrangler deploy`. Only pushes to `main` deploy; there is no manual dispatch. `.github/workflows/warmup.yml` runs the warmup script hourly on a cron. `.github/workflows/pr.yml` runs typecheck and tests on every PR.
